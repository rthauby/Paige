(function() {
  var base_config, check_for_docco, clean_file_extension, clean_path_names, config_template, configuration, ensure_directory, exec, fs, get_subfiles, mdown_template, paige_background, path, process_config, process_docco_files, process_docco_wrappers, process_html_file, read_config, showdown, spawn, template, wrapper_template, _, _ref;
  ...
  get_subfiles = function(callback) {
    return exec("ls " + configuration.docco_files, function(error, stdout, stderr) {
      if (callback) {
        return callback(stdout.trim().split("\n"));
      }
    });
  };
